...
### 2026-04-11 (`0.2.5` dark mode history fix)

- Goal: Fix unreadable source-file history highlights in dark mode with the smallest possible UI change.
- Files/areas changed: `document_control/mixins/records.py`, `tests/test_v010_features.py`, `docs/0.2.5/dark-mode-history-fix-implementation-plan.md`, `docs/features.md`, `docs/working-memory.md`.
- Behavior changed: Source-file history highlights keep their existing background meanings, but now also force a dark foreground color so highlighted filenames remain readable in dark mode.
- Tests run: Targeted pytest and repo-local `py_compile` planned for the touched files in this environment.
- Risks or follow-up: This is an intentionally narrow fix for the source-file history highlight path; if later theme issues appear in other custom-highlighted tables, those should be handled as separate palette/readability follow-up work.
